...
6.- Sacar aquellos nombres mayores que el texto introducido
7.- Salir
 */

using System;
public class AgendaV5
{
	public static void Main()
	{
		...
		while (!salir)
		{
			...
			Console.WriteLine("6.- Sacar aquellos nombres mayores que el texto introducido");
			...
			switch (eleccion)
			{
				...
				case '5':
					for (int i = 0; i < agenda.Length; i++)
					{
						...
						{
							Console.Write(nombreSplit[j].Substring(0, 1).ToLower() + nombreSplit[j].Substring(1, nombreSplit[j].Length - 1) + " ");
						}
						Console.WriteLine();
					}
					Console.WriteLine();
				...
				case '6':
					...
					for (int i = 0; i < agenda.Length; i++)
					{
						if (String.Compare(agenda[i], entradaUsuario, true) > 0)
						{
